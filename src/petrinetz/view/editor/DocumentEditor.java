...
import petrinetz.view.editor.listeners.CanvasMouseAddEdgeListener;
import petrinetz.view.editor.listeners.CanvasMouseAddTransitionListener;
import petrinetz.view.editor.listeners.CanvasMouseAddPlaceListener;
import petrinetz.view.editor.listeners.CanvasMouseSelectionListener;
...
import petrinetz.view.editor.listeners.CanvasMouseSetPropertiesListener;
import petrinetz.view.editor.listeners.CanvasMouseSimulationListener;
...
public class DocumentEditor extends JPanel {
     ...
     *  AddPlace - add or move places.<br/>
     *  AddTransition - add or move transitions<br/>
     *  AddEdge - add or move edges<br/>
     *  SetProperties - set the properties of the entity or edge, that is being clicked on.<br/>
     *  Simulation - simulation mode
     */
    public enum EditorMode {
        /**
         * In selection mode, a click on an entity means sets the entity to the only selection. Ctrl+click and Shift+click means add or remove the entity from the selection. Alt+click means remove from selection. When dragging the mouse on the background we can select multiple entities in a rectangular area. Modifier keys also work in rectangular selection mode.
         */
        Selection,
        /**
         * In addplace mode, a click creates a new place in the Petri net. Dragging existing places will move them and the selection. Selection modifier keys will also work here.
         */
        AddPlace,
        /**
         * In addtransition mode, a click creates a new transition in the Petri net. Dragging existing transitions will move them and the selection. Selection modifier keys will also work here.
         */
        AddTransition,
        /**
        ...
        AddEdge,
        /**
         * A new editor dialog pops up when clicking an entity or edge. the clicked objects properties can be set in the editor dialog.
         */
        SetProperties,
        /**
         * Simulation mode. When a transition is executable, it will turn red. Clicking it will execute the transition and add or remove the resources from the source and destination places.
         */
        Simulation
    };
    ...
    private PetriNetDocument _document;
        ...
        if(canvas != null) {
            switch(value) {
                case Selection:
                    ...
                    break;
                case AddPlace:
                    _currentMouseListener = new CanvasMouseAddPlaceListener(this);
                    break;
                case AddTransition:
                    _currentMouseListener = new CanvasMouseAddTransitionListener(this);
                    break;
                case AddEdge:
                    _currentMouseListener = new CanvasMouseAddEdgeListener(this);
                    break;
                case SetProperties:
                    _currentMouseListener = new CanvasMouseSetPropertiesListener(this);
                    break;
                case Simulation:
                    _currentMouseListener = new CanvasMouseSimulationListener(this);
                    break;
            }
        }
     ...
     */
    public DocumentEditor(PetriNetDocument document) {
    ...
    // <editor-fold defaultstate="opened" desc="Getter/setter methods">
    ...
    /**
     * Getter to determine whether there is a rectangular selection going on or not.
     */
     ...
     * @param value
     */
    public void setDocument(PetriNetDocument value) {
        _document = value;
     ...
     * @param value
     */
    public PetriNetDocument getDocument() {
        return _document;
    }
    ...
    /**
     * Selection setter.
     * @param selection the entity that will be the only selected one.
     */
     ...
     * @param selection the entities to select.
     */
    ...
    /**
     * Adds an entity to the selection if it isn't currently selected, otherwise removes it's selection status.
     * @param selection
     */
    public void addToOrRemoveFromSelection(Entity selection) {
        if(_selectedEntities.containsKey(selection.sign))
            removeFromSelection(selection);
        else addToSelection(selection);
    }

    /**
     * Adds multiple entities to the selection if one isn't currently selected, otherwise removes it's selection status.
     * @param selection
     */
    ...
    // <editor-fold defaultstate="opened" desc="Event handler classes">
